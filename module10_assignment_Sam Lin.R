  ...
  sidebarLayout(
    ...
    sidebarPanel(
      ...
      selectInput(inputId = "subset_income",
                  ...
                  label = "Select Subset Income:"), # 2, 3
      # Interactive piece 2: inputId = "set_yaxis"
      selectInput(inputId = "set_yaxis",
                  choices = c("Capital Loss"= "capital_loss", "Hours per Week" = "hours_per_week"),
                  label = "y axis based on:"), # 4, 5
      ...
      checkboxGroupInput(inputId = "subset_occupation",
                         ...
                         label = "Select Occupation:")), # 6, 7
    # Main panel
    mainPanel(plotOutput(outputId = "myfigure")) # 8
  )
)
...
#                                     income %in% input$income & # Why is this wrong?
                                     occupation %in% input$subset_occupation)) # 9
  
  # Render Plot
  output$myfigure <- renderPlot(ggplot(create_subset()) + # 10, 11
                                  # Boxplot of x = occupation, y = defined by input
                                  geom_boxplot(aes_string(x = "occupation", y =input$set_yaxis)) + # 12, 13
                                  theme_bw(18) +
